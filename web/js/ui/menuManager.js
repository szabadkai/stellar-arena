...
class MenuManager {
    ...
    setupEventListeners() {
        ...
        // Shipyard
        document.getElementById('launch-battle-btn').addEventListener('click', () => {
            if (this.sound) this.sound.playClick();
            this.launchBattle();
        });
        ...
        // Post-battle
        document.getElementById('continue-campaign-btn').addEventListener('click', () => {
            if (this.sound) this.sound.playClick();
            this.continueFromBattle();
        });

        document.getElementById('abandon-campaign-btn').addEventListener('click', () => {
            if (this.sound) this.sound.playClick();
            this.abandonCampaign();
        });

        // Upgrades
        document.getElementById('finish-upgrades-btn').addEventListener('click', () => {
            if (this.sound) this.sound.playClick();
            this.finishUpgrades();
        });
    }
    ...
    updateShipyard() {
        ...
        document.getElementById('credits-display').textContent = this.progression.credits;
        document.getElementById('battle-number').textContent = this.campaign.battleNumber;
        ...
        if (ship.hull < ship.maxHull) {
            ...
            repairBtn.addEventListener('click', (e) => {
                ...
                if (this.progression.repairShip(index, ship.maxHull - ship.hull)) {
                    ...
                    this.modal.alert('Insufficient Credits', 'You don\'t have enough credits to repair this ship.');
                }
            });
    ...
    createShopCard(shipType, preset) {
        ...
        buyBtn.addEventListener('click', (e) => {
            ...
            if (this.progression.buyShip(shipType)) {
                ...
                this.modal.alert('Insufficient Credits', `You need ${preset.creditCost}₡ to purchase this ship.`);
            }
        });
    ...
    createShipCard(ship) {
        ...
        const spriteCanvas = this.spriteGenerator.generateShipSprite(ship.shipClass, ship.team);
    ...
    launchBattle() {
        ...
        if (!ownedShips || ownedShips.length === 0) {
            if (this.modal) {
                this.modal.alert('No Ships', 'Purchase at least one ship before launching into battle!');
            }
            return;
        }
        ...
        ships.forEach((ship, shipIndex) => {
            ...
            upgradeChoices.forEach(upgrade => {
                ...
                card.addEventListener('click', () => {
                    ...
                    choicesDiv.querySelectorAll('.upgrade-card').forEach(c => c.classList.remove('selected'));
    ...
    finishUpgrades() {
        ...
        if (hasNext) {
            ...
            // Campaign complete!
            if (this.modal) {
                this.modal.alert('Campaign Complete!', 'Congratulations, Commander! You have achieved total victory!').then(() => {
                    this.campaign.reset();
                    this.showScreen('main-menu');
                    this.updateMainMenu();
                });
    ...
    async abandonCampaign() {
        if (this.modal) {
            const confirmed = await this.modal.confirm('Abandon Campaign?', 'Are you sure you want to abandon this campaign? All progress will be lost.');
            if (confirmed) {
